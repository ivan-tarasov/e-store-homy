...
$thumb_square_size  = 128;
// Максимальный размер изображения
$max_image_size 	  = 256;
// Префикс обрезанного изображения
$thumb_prefix		  = "_avatar";
// Каталог для загрузки
//$destination_folder = $_SERVER['DOCUMENT_ROOT'].'/public/avatar/';
$destination_folder = '../../../'.STATIC_URL.AVATAR_PATH;
//error_log(__FILE__.' :: '.$destination_folder.' :: '.realpath($destination_folder));
...
if (isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	if($image_res){
		...
		$new_file_name = md5($image_name_only) . '.' . $image_extension;
		...
		//if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality)) {
			...
			if(!crop_image_square($image_res, $thumb_save_folder, $image_type, $thumb_square_size, $image_width, $image_height, $jpeg_quality))
				die('Ошибка при создании аватара');

			$update['avatar'] = $new_file_name;
			$db->update('users',$update,'id='.$user_id);

			/* We have succesfully resized and created thumbnail image
			We can now output image to user's browser or store information in the database*/
			echo '<div align="center">';
			echo '<img src="'.HTTP_PROTOCOL.STATIC_URL.AVATAR_PATH.$new_file_name.'" alt="Новый аватар">';
			echo '</div>';
		//}
